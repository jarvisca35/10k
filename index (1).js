...
// Monday 6:30 AM - Weekly setup checklist
cron.schedule('30 6 * * 1', () => {
  send(formatChecklist(mondayChecklist, '📋 Monday Setup Checklist:'));
}, { timezone: TZ });

// Monday 6:35 AM - New weekly challenge drops
cron.schedule('35 6 * * 1', () => {
  send(formatChallenge());
}, { timezone: TZ });

// Wednesday 12:30 PM - Midweek challenge nudge
cron.schedule('30 12 * * 3', () => {
  const now = new Date(new Date().toLocaleString('en-US', { timeZone: TZ }));
  send(`⚡ Midweek check — still holding the challenge?\n\n${currentChallenge(now).title}`);
}, { timezone: TZ });

// Sunday 8:15 PM - Challenge results + week reflection
